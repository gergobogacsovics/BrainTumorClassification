...
from net import get_base_network, get_base_network_output_dim, BaseNetworkWrapperL2Normed, CNNEnsemble_ConvFeaturesOnly, BaseNetworkWrapperOriginal
...
ensemble_histogram_nbins = config["base"]["ensemble"]["histogram_nbins"]
...
if ensemble_or_base == "base":
    EXPERIMENT_NAME = f"distinct_base_{base_network_1}_{mode}"
else:
    EXPERIMENT_NAME = f"distinct_ensemble_{mode}"


EXPERIMENT_SAVE_DIR = f"experiments/{EXPERIMENT_NAME}_{img_size}_{int(time.time())}"
...
if num_channels == 3:
    transforms_main_train = T.Compose([
        ...
        T.ToTensor(),
        T.Normalize((0,0,0), (1,1,1)) # TODO?
    ])
else:
    transforms_main_train = T.Compose([
        T.Resize((int(img_size * 1.1), int(img_size * 1.1))),
        T.ToTensor(),
        T.Normalize((0), (1)) # TODO?
    ])
...
if ensemble_or_base == "base":
    ...
    model_1 = BaseNetworkWrapperL2Normed(base_network=base_network_1, num_channels=num_channels, num_classes=num_classes, img_size=img_size, pretrained=pretrained)
    model_1.to(device)
    model_2 = BaseNetworkWrapperL2Normed(base_network=base_network_2, num_channels=num_channels, num_classes=num_classes, img_size=img_size, pretrained=pretrained)
    model_2.to(device)
    model_3 = BaseNetworkWrapperL2Normed(base_network=base_network_3, num_channels=num_channels, num_classes=num_classes, img_size=img_size, pretrained=pretrained)
    model_3.to(device)
...
def train():
    ...
    criterion = nn.CrossEntropyLoss(weight=class_weights_calc)
    criterion_histogram_loss = HistogramLoss(num_steps=ensemble_histogram_nbins, cuda=True)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if ensemble_or_base == "base":
            ...
            for _, images, labels in train_loader:
                ...
                features_combined = torch.vstack((features_1, features_2, features_3))
                comb_size = images.shape[0]
                group_labels_combined = torch.hstack((torch.tensor([0]).repeat(comb_size), torch.tensor([1]).repeat(comb_size), torch.tensor([2]).repeat(comb_size)))
                group_labels_combined = group_labels_combined.to(device)
                loss_similarity = ensemble_lambda * criterion_histogram_loss(features_combined, group_labels_combined)
